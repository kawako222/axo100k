...
export default function DashboardAxoLabs() {
  const [prospects, setProspects] = useState(() => {
    const savedProspects = localStorage.getItem('axo_prospects');
    return savedProspects ? JSON.parse(savedProspects) : [];
  });
  ...
  useEffect(() => {
    localStorage.setItem('axo_prospects', JSON.stringify(prospects));
    localStorage.setItem('axo_messages', messagesSent.toString());
  }, [prospects, messagesSent]);
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.id) {
      ...
      setProspects([...prospects, { ...formData, id: Date.now().toString() }]);
    }
